...
float	determine_hex_corners(t_img *canvas, float (*corners)[6][2], int center_x, int center_y)
{
	float angle_deg;
	float angle_rad;
	int i = 0;
	...
	while (i < 6)
	{
		...
		(*corners)[i][0] = center_x + g_size * cos(angle_rad);
		(*corners)[i][1] = center_y + g_size * sin(angle_rad);
...
int	draw_one_hex(t_img *canvas, int center_x, int center_y)
{
	...
	determine_hex_corners(canvas, &corners, center_x, center_y);
	printf("center x = %d\n", center_x);
	printf("center y = %d\n", center_y);
